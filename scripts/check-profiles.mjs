...
const store=JSON.parse(await readFile(join(homedir(),'.draey-codex-hub','hub.json'),'utf8'));
const binary=resolve(process.argv[2]||'src-tauri/target/debug/examples/verify_native.exe');
await Promise.allSettled(store.profiles.map(profile=>new Promise(resolve=>{
 const child=spawn(binary,['profile-usage',profile.id],{windowsHide:true,stdio:['ignore','pipe','pipe']});let output='',error='';
