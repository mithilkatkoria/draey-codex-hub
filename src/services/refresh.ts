...
export class RefreshCoordinator {
 ...
 refresh(id:string): Promise<void> {
  ...
  const operation=(async()=>{try{this.publish(id,await this.pool.run(()=>this.fetch(id)));}catch(error){this.publish(id,{windows:[],fetchedAt:'',source:'Codex app-server',...before,state:'error',message:String(error instanceof Error?error.message:error)});}finally{this.pending.delete(id);}})();
