...
export function WorkspaceNotice({workspace,profiles,onCancel}:{workspace?:WorkspaceStatus;profiles:Profile[];onCancel:()=>void}) {
 ...
 useEffect(()=>{if(workspace?.pending?.stage==='waiting'){notice.current?.scrollIntoView?.({block:'start'});notice.current?.focus({preventScroll:true});}},[workspace?.pending?.stage]);
 ...
 return <section ref={notice} tabIndex={-1} className={`workspace-notice ${pending?'switch-pending':''}`} role={pending?'status':undefined}>
  ...
  <div><strong>{pending?(pending.stage==='waiting'?'Your account switch is ready':'Preparing your workspace'):'Your projects. Your tasks. The same Codex.'}</strong>
  ...
  {pending?.stage==='waiting'?<button className="secondary" onClick={onCancel}><X size={14}/> Cancel switch</button>:!pending&&<span className="workspace-current">{current?`Current · ${current.name}`:'Existing workspace'}</span>}
 </section>;
}
