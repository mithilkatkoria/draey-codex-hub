import { AccountSculpture } from './AccountSculpture';
...
export const ProfileCard=memo(function ProfileCard({profile:p,snapshot,settings,now,active=false,busy,onLaunch,onRefresh,onEdit,onLogin}:Props) {
 ...
 function meter(w:UsageWindow,ring=true) {
  ...
  return <div className={`quota ${warning?'quota-low':''} ${ring?'quota-with-ring':'quota-wide'}`} key={w.id} style={{'--quota-value':value??0} as CSSProperties}>
   {ring&&<div className="quota-ring" aria-hidden="true"><svg viewBox="0 0 80 80"><circle className="ring-track" cx="40" cy="40" r="34"/><circle className="ring-value" cx="40" cy="40" r="34" pathLength="100" strokeDasharray={`${value??0} 100`}/></svg><strong>{value==null?'N/A':`${Number(value.toFixed(1))}%`}</strong></div>}
   <div className="quota-copy"><div className="quota-heading"><span>{w.label}</span>{!ring&&<strong>{value==null?'N/A':`${Number(value.toFixed(1))}%`}</strong>}</div><span className="quota-caption">remaining{cached?' · cached':''}</span>
    <div className="quota-segments" role="meter" aria-label={`${w.bucket} ${w.label} remaining`} aria-valuenow={value??undefined} aria-valuemin={0} aria-valuemax={100} aria-valuetext={value==null?'Not reported':`${Number(value.toFixed(1))}% remaining`}><i style={{width:`${value??0}%`}}/></div>
    <div className="quota-detail">{w.usedPercent==null?'Used: not reported':`${Number(w.usedPercent.toFixed(1))}% used`}{w.durationMins!=null&&<span>{w.durationMins>=1440?`${w.durationMins/1440}-day window`:`${w.durationMins/60}-hour window`}</span>}</div><div className="quota-reset" title={resetDate(w.resetsAt)}><Clock3 size={12}/><span>{w.resetsAt!=null&&w.resetsAt*1000>now?'Resets in ':''}{resetRemaining(w.resetsAt,now)}</span></div><time className="quota-date" dateTime={w.resetsAt==null?undefined:new Date(w.resetsAt*1000).toISOString()}>{w.resetsAt==null?'Reset date not reported':resetDate(w.resetsAt)}</time>
   </div>
  </div>;
 }
 return <article className={`profile-card account-tile accent-${p.accent} ${active?'is-current':''} ${reserved?'reserved':''} ${auth?'needs-auth':''}`} aria-label={name}>
  ...
  <div className="portrait-art" aria-hidden="true"><div className="art-halo"/><AccountSculpture kind={plan.toLowerCase().startsWith('pro')?'crown':p.accent==='mint'?'cubes':p.accent==='blue'?'orbit':'ribbon'}/></div>
  ...
  <div className={`tile-quotas ${core.length===1?'single-quota':''}`}>{core.length?core.map(w=>meter(w,true)):<div className="usage-empty"><strong>{state==='refreshing'?'Checking allowances':auth?'Connect your account':'Usage unavailable'}</strong><span>{auth?'Sign in to retrieve limits.':'No limits reported yet.'}</span></div>}</div>
